...
class Disciplina_Controller {
    ...
    async cadastrar_disciplina(req, resp) {
        ...
        const nomeValido = /^[a-zA-Z\s]*\d?[a-zA-Z\s]*$/.test(nome);
        ...
        if (!nome || !temLetra || !nomeValido) {
            return resp.send({ ok: false, msg: "Nome inválido: deve conter apenas letras e no máximo 1 número." });
        }

        if (!cargaValida) {
            return resp.send({ ok: false, msg: "Carga horária inválida: deve ser número inteiro positivo." });
        }
        ...
        if (disciplinaExiste) {
            return resp.send({ ok: false, msg: "Já existe uma disciplina cadastrada com esse nome." });
        }
    ...
    async editar_disciplina(req, resp) {
        ...
        let disciplinaExiste = lista_disciplina.some(d =>
            d.nome.toLowerCase() === nome.toLowerCase() && d.id !== Number(req.body.id)
        );
        if (disciplinaExiste) {
            return resp.send({ ok: false, msg: "Já existe uma disciplina cadastrada com esse nome." });
        }
